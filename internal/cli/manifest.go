...
func commandInfos() []manifest.CommandInfo {
	return []manifest.CommandInfo{
		...
		{Name: "schemas", Summary: "列出 schema", Usage: "dbm-cli schemas -d <ds> [-l PATTERN]",
			Flags:    []manifest.Flag{{Name: "like", Shorthand: "l", Usage: "名称模糊匹配"}},
			Examples: []string{"dbm-cli schemas -d prod-ro -l HR%"}},
		{Name: "tables", Summary: "列出表", Usage: "dbm-cli tables -d <ds> [-s S] [-l P]",
			Flags: []manifest.Flag{
				{Name: "schema", Shorthand: "s", Usage: "schema 名，默认当前用户"},
				{Name: "like", Shorthand: "l", Usage: "表名模糊匹配"},
			},
			Examples: []string{"dbm-cli tables -d prod-ro -s HR"}},
		{Name: "columns", Summary: "查看表结构", Usage: "dbm-cli columns -d <ds> -t T [-s S]",
			Flags: []manifest.Flag{
				{Name: "table", Shorthand: "t", Usage: "表名（必填）"},
				{Name: "schema", Shorthand: "s", Usage: "schema 名"},
			},
			Examples: []string{"dbm-cli columns -d prod-ro -t EMPLOYEES -s HR"}},
		{Name: "indexes", Summary: "查看表索引", Usage: "dbm-cli indexes -d <ds> -t T [-s S]",
			Flags: []manifest.Flag{
				{Name: "table", Shorthand: "t", Usage: "表名（必填）"},
				{Name: "schema", Shorthand: "s", Usage: "schema 名"},
			},
			Examples: []string{"dbm-cli indexes -d prod-ro -t EMPLOYEES"}},
		{Name: "views", Summary: "列出视图", Usage: "dbm-cli views -d <ds> [-s S]",
			Flags:    []manifest.Flag{{Name: "schema", Shorthand: "s", Usage: "schema 名"}},
			Examples: []string{"dbm-cli views -d prod-ro -s HR"}},
		{Name: "table", Summary: "分页查表数据", Usage: "dbm-cli table -d <ds> -t T [-s S] [-n N] [--offset N] [--order COL]",
			Flags: []manifest.Flag{
				{Name: "name", Shorthand: "t", Usage: "表名（必填）"},
				{Name: "schema", Shorthand: "s", Usage: "schema 名"},
				{Name: "limit", Shorthand: "n", Default: "100", Usage: "返回行数"},
				{Name: "offset", Default: "0", Usage: "跳过行数"},
				{Name: "order", Usage: "排序列"},
			},
			Examples: []string{"dbm-cli table -d prod-ro -t EMPLOYEES -s HR -n 20 -o json"}},
		{Name: "query", Summary: "执行任意 SQL（只读直执行；写受 allow_write 守卫；支持文件/stdin/参数绑定）",
			Usage: "dbm-cli query -d <ds> \"SQL\" [-f F] [--param V ...] [-n N] [--yes]",
			Flags: []manifest.Flag{
				{Name: "file", Shorthand: "f", Usage: "从文件读取 SQL（优先级高于 stdin 与命令行参数）"},
				{Name: "param", Usage: "绑定到 ? 占位符的参数值，按顺序，可多次指定；自动按引擎转换为 ?/$1/:1"},
				{Name: "limit", Shorthand: "n", Default: "1000", Usage: "只读查询返回的最大行数（<=0 不限制，防 SELECT * 拉爆大表）"},
			...
			Examples: []string{
				...
				"dbm-cli query -d prod-ro -f /path/to/report.sql -n 500",
