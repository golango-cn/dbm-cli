...
func (c *conn) QueryTable(ctx context.Context, schema, table string, limit, offset int64, orderCol string) (*driver.Result, error) {
	if table == "" {
		return nil, fmt.Errorf("clickhouse: QueryTable: table name is required")
	}
	if schema == "" {
		schema = c.cfg.Database
	}
	if schema == "" {
		return nil, fmt.Errorf("clickhouse: QueryTable: no database selected; please specify --schema or set 'database' in config")
	}
	if limit <= 0 {
		limit = 100
	}
